...
def fragment_prediction(sentence, filename): #filename se paseaza cu extensia .json
    ...
    yaml_file = open(os.path.join(dataDir,filename + "model.yaml"), 'r')
    loaded_model_yaml = yaml_file.read()
    yaml_file.close()

    loaded_model = model_from_yaml(loaded_model_yaml)
    # load weights into new model
    loaded_model.load_weights(os.path.join(dataDir,filename + "model.h5"))
    ...
    return book_info[str(prediction[0])]
